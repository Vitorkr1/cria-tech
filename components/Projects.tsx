...
function ProjectCard({
  ...
  return (
    ...
    >
      <TiltCard
        strength={3}
        className="group relative overflow-hidden rounded-2xl border border-brand-border bg-white transition-colors hover:border-brand-blue/30"
      >
        {project.isNew && (
        ...
        <div className="relative h-[200px] overflow-hidden bg-brand-surfaceAlt2">
          {project.image ? (
            <Image
              ...
              className="object-cover transition-transform duration-500 group-hover:scale-110 group-hover:[animation:crt-warmup_0.9s_ease-out_1]"
            />
          ) : (
            <div className="h-full w-full transition-transform duration-500 group-hover:scale-110 group-hover:[animation:crt-warmup_0.9s_ease-out_1]">
              <ProjectPreview id={project.id} />
            </div>
          )}
          {/* scanline "ligando a tela" */}
          <span
            aria-hidden="true"
            className="pointer-events-none absolute inset-x-0 top-full h-2/5 bg-gradient-to-b from-transparent via-brand-cyan/60 to-transparent opacity-0 mix-blend-screen group-hover:[animation:crt-scan_0.7s_ease-out_1]"
          />
          {/* vinheta escura que "acende" junto */}
          <span
            aria-hidden="true"
            className="pointer-events-none absolute inset-0 opacity-0 [background:radial-gradient(ellipse_at_center,transparent_35%,rgba(0,0,0,0.55)_140%)] group-hover:[animation:crt-flicker-vignette_0.9s_ease-out_1]"
          />
...
export default function Projects() {
  ...
  return (
    <section id="projetos" className="relative overflow-hidden bg-transparent px-4 py-24 sm:px-8">

      <div className="relative mx-auto max-w-[1200px]">
        <Reveal>
          <div className="mb-3 mt-4 text-xs font-semibold uppercase tracking-widest text-brand-blue">Nosso trabalho</div>
          <h2 className="mb-4 font-sans text-4xl font-extrabold tracking-tight text-brand-ink sm:text-5xl">Projetos realizados</h2>
        ...
        <Reveal delay={0.1}>
          <div className="mb-10 flex flex-nowrap gap-2 overflow-x-auto pt-2 pb-2 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:gap-3">
            {PROJECT_FILTERS.map((f) => (
              <button
                ...
                className={`shrink-0 whitespace-nowrap rounded-full border px-4 py-2 text-sm font-semibold transition-all sm:px-5 sm:py-2.5 ${
                  filter === f.value
                    ? '-translate-y-0.5 border-brand-blue bg-brand-blue text-white'
